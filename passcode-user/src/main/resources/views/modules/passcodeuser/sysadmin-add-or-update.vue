    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/passcodeuser/sysadmin/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.sysAdmin.name
                this.dataForm.password = data.sysAdmin.password
                this.dataForm.sex = data.sysAdmin.sex
                this.dataForm.age = data.sysAdmin.age
                this.dataForm.email = data.sysAdmin.email
                this.dataForm.adminhead = data.sysAdmin.adminhead
                this.dataForm.mobile = data.sysAdmin.mobile
                this.dataForm.status = data.sysAdmin.status
                this.dataForm.remark = data.sysAdmin.remark
                this.dataForm.lastlogintime = data.sysAdmin.lastlogintime
                this.dataForm.modifiedtime = data.sysAdmin.modifiedtime
                this.dataForm.createtime = data.sysAdmin.createtime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/passcodeuser/sysadmin/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
