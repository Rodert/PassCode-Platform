    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/passcodeuser/members/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.members.name
                this.dataForm.password = data.members.password
                this.dataForm.sex = data.members.sex
                this.dataForm.age = data.members.age
                this.dataForm.email = data.members.email
                this.dataForm.adminhead = data.members.adminhead
                this.dataForm.mobile = data.members.mobile
                this.dataForm.status = data.members.status
                this.dataForm.remark = data.members.remark
                this.dataForm.lastlogintime = data.members.lastlogintime
                this.dataForm.modifiedtime = data.members.modifiedtime
                this.dataForm.createtime = data.members.createtime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/passcodeuser/members/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
